...
class DistributionKernel(Kern):
    ...
    def K(self, X, X2):
        ...
        Distribution_p1 = np.ascontiguousarray(
            X[:, dimension_x:(k+dimension_x)])
        Distribution_theta1 = np.ascontiguousarray(
            X[:, (k+dimension_x):(2*k+dimension_x)])
        Distribution_p2 = np.ascontiguousarray(
            X2[:, dimension_x:(k+dimension_x)])
        Distribution_theta2 = np.ascontiguousarray(
            X2[:, (k+dimension_x):(2*k+dimension_x)])
        ...
        Distribution_p3 = np.ascontiguousarray(
            X[:, (2*k+dimension_x):(3*k+dimension_x)])
        Distribution_theta3 = np.ascontiguousarray(
            X[:, (3*k+dimension_x):(4*k+dimension_x)])
        Distribution_p4 = np.ascontiguousarray(
            X2[:, (2*k+dimension_x):(3*k+dimension_x)])
        Distribution_theta4 = np.ascontiguousarray(
            X2[:, (3*k+dimension_x):(4*k+dimension_x)])
        ...
        Distribution_p5 = np.ascontiguousarray(
            X[:, (4*k+dimension_x):(5*k+dimension_x)])
        Distribution_theta5 = np.ascontiguousarray(
            X[:, (5*k+dimension_x):(6*k+dimension_x)])
        Distribution_p6 = np.ascontiguousarray(
            X2[:, (4*k+dimension_x):(5*k+dimension_x)])
        Distribution_theta6 = np.ascontiguousarray(
            X2[:, (5*k+dimension_x):(6*k+dimension_x)])
        ...
        Distribution_p7 = np.ascontiguousarray(
            X[:, (6*k+dimension_x):(7*k+dimension_x)])
        Distribution_theta7 = np.ascontiguousarray(
            X[:, (7*k+dimension_x):(8*k+dimension_x)])
        Distribution_p8 = np.ascontiguousarray(
            X2[:, (6*k+dimension_x):(7*k+dimension_x)])
        Distribution_theta8 = np.ascontiguousarray(
            X2[:, (7*k+dimension_x):(8*k+dimension_x)])
        ...
        Distribution_p9 = np.ascontiguousarray(
            X[:, (8*k+dimension_x):(9*k+dimension_x)])
        Distribution_theta9 = np.ascontiguousarray(
            X[:, (9*k+dimension_x):(10*k+dimension_x)])
        Distribution_p10 = np.ascontiguousarray(
            X2[:, (8*k+dimension_x):(9*k+dimension_x)])
        Distribution_theta10 = np.ascontiguousarray(
            X2[:, (9*k+dimension_x):(10*k+dimension_x)])
        ...
        Distribution_p11 = np.ascontiguousarray(
            X[:, (10*k+dimension_x):(11*k+dimension_x)])
        Distribution_theta11 = np.ascontiguousarray(
            X[:, (11*k+dimension_x):(12*k+dimension_x)])
        Distribution_p12 = np.ascontiguousarray(
            X2[:, (10*k+dimension_x):(11*k+dimension_x)])
        Distribution_theta12 = np.ascontiguousarray(
            X2[:, (11*k+dimension_x):(12*k+dimension_x)])
    ...
    def update_gradients_full(self, dL_dK, X, X2):
        ...
        correlation = correlation1x * correlation2x * correlation3x * correlation1p * \
            correlation2p * correlation3p * correlation4p * correlation5p * correlation6p
        dvar = correlation
        dl1x = self.variance * correlation * \
            0.5 * dist1x/(self.lengthscale1x**2)
        dl2x = self.variance * correlation * \
            0.5 * dist2x/(self.lengthscale2x**2)
        dl3x = self.variance * correlation * \
            0.5 * dist3x/(self.lengthscale3x**2)
        dl1p = self.variance * correlation * \
            0.5 * dist1p/(self.lengthscale1p**2)
        dl2p = self.variance * correlation * \
            0.5 * dist2p/(self.lengthscale2p**2)
        dl3p = self.variance * correlation * \
            0.5 * dist3p/(self.lengthscale3p**2)
        dl4p = self.variance * correlation * \
            0.5 * dist4p/(self.lengthscale4p**2)
        dl5p = self.variance * correlation * \
            0.5 * dist5p/(self.lengthscale5p**2)
        dl6p = self.variance * correlation * \
            0.5 * dist6p/(self.lengthscale6p**2)
...
def GP_model(D, Y, method=None):
    ...
    if method == "NBRO":
        ...
        kernel = DistributionKernel(
            input_dim=D.shape[1], variance=1., lengthscale=1., power=1)
...
def inital_design(n_sample, seed):
    ...
    X_all = candidate[((candidate[:, 0] + candidate[:, 1] + candidate[:, 2]/2) <= 28)
...
def NBRO(seed):
    ...
    # this is different
    # initial experiment design
    X1, D1 = candidate_space(
        xi,  n_sample, n_k, dimension_p, P0_list, seed=seed, alpha=alpha)
    Y_list = list()
    for i in range(D1.shape[0]):
        D_selected = D1[i]
    ...
    # this is different
    # get prediction and obtain minimizer
    X_, D_ = candidate_space(xi, n_candidate, n_k,
                             dimension_p, P0_list, seed=None, alpha=alpha)
...
parser.add_argument('-t', '--time', type=str,
                    help='Date of the experiments, e.g. 20201109', default='20240824')
...
parser.add_argument('-problem', '--problem', type=str,
                    help='Function', default='ccf')

parser.add_argument('-smacro', '--start_num', type=int,
                    help='Start number of macroreplication', default=12)
parser.add_argument('-macro', '--repet_num', type=int,
                    help='Number of macroreplication', default=24)
...
parser.add_argument('-n_MC', '--n_MC', type=int,
                    help='Number of Monte Carlo samples for the posterior', default=50)
...
true_dist = [
    ...
    {'dist': "m-lognorm", 'mu': [15.0, 30],
        'sigma': [7, 7], 'p_i': [0.5, 0.5]},
    {'dist': "m-lognorm", 'mu': [3.4, 6.8],
        'sigma': [3.5, 3.5], 'p_i': [0.5, 0.5]},
    {'dist': "m-lognorm", 'mu': [3.8, 7.6],
        'sigma': [1.6, 1.6], 'p_i': [0.5, 0.5]},
...
file_pre = '_'.join(["outputs/bo", time_run, "n_rep",
                    str(n_rep), problem, method, true_dist[2].get('dist')])
print(file_pre)
