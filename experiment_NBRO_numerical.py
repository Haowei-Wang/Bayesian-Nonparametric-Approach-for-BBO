...
class DistributionKernel(Kern):
    ...
    def K(self, X, X2):
        ...
        Distribution_theta1 = np.ascontiguousarray(X[:, (k+2):])/1.0
        Distribution_p2 = np.ascontiguousarray(X2[:, 2:(k+2)])
        Distribution_theta2 = np.ascontiguousarray(X2[:, (k+2):])/1.0
        N1 = X.shape[0]
        N2 = X2.shape[0]
...
def NBRO(seed):
    ...
    for i in range(n_iteration):
        ...
        # print(D_new.shape) #(dimension_x + dimension_y, )
...
parser.add_argument('-t', '--time', type=str,
                    help='Date of the experiments, e.g. 20201109', default='20240912')
...
parser.add_argument('-problem', '--problem', type=str,
                    help='Function', default='Griewank')  # StybTang

parser.add_argument('-smacro', '--start_num', type=int,
                    help='Start number of macroreplication', default=24)
parser.add_argument('-macro', '--repet_num', type=int,
                    help='Number of macroreplication', default=48)
...
parser.add_argument('-n_i', '--n_iteration', type=int,
                    help='Number of iteration', default=40)
...
parser.add_argument('-alpha', '--alpha', type=float, help='alpha', default=10)

parser.add_argument('-noise', '--noise', type=float, help='noise', default=0.1)
...
if args.true_distribution == 'm-lognorm':
    if problem == "inventory":
        true_dist = [{'dist': "m-lognorm",
                      ...
                      'sigma': [5000, 5000],
                      'p_i': [0.5, 0.5]}]
    elif problem == "StybTang":
        true_dist = [{'dist': "m-lognorm",
                      'mu': [2, 4],
                      'sigma': [2, 1], 'p_i': [0.5, 0.5]}]
    else:
        true_dist = [{'dist': "m-lognorm",
                      'mu': [10, 20],
...
# %%
if problem == "StybTang":
    f = StybTang(true_dist, std_noise)
elif problem == "Griewank":
    f = Griewank(true_dist, std_noise)
...
file_pre = '_'.join(["outputs/bo", time_run, "alpha", str(alpha), "n_rep", str(
    n_rep), "std", str(std_noise), problem, method, true_dist[0].get('dist')])
file_pre = file_pre.replace(".", "_")
